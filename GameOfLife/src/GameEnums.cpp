...
std::string gol::Actions::ToString(ActionVariant action)
{
	std::string result{ [action]()
	{
		...
		if (auto* value = std::get_if<EditorAction>(&action))
			return std::find_if(EditorActionDefinitions.begin(), EditorActionDefinitions.end(),
				[value](auto&& pair) { return pair.second == *value; })->first;
		if (auto* value = std::get_if<SelectionAction>(&action))
			return std::find_if(SelectionActionDefinitions.begin(), SelectionActionDefinitions.end(),
				[value](auto&& pair) { return pair.second == *value; })->first;
		std::unreachable();
	}() };
