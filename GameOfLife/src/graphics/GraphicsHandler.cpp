    ...
    , m_Shader(shaderFilePath)
{
    ...
    GL_DEBUG(glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, m_Texture.ID(), 0));
    ...
    GL_DEBUG(glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_RENDERBUFFER, m_renderBuffer.ID()));
...
{
    GL_DEBUG(glBindTexture(GL_TEXTURE_2D, m_Texture.ID()));
    GL_DEBUG(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, windowBounds.Width, windowBounds.Height, 0, GL_RGB, GL_UNSIGNED_BYTE, NULL));
    GL_DEBUG(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR));
    GL_DEBUG(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR));
    ...
    GL_DEBUG(glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, m_Texture.ID(), 0));
    GL_DEBUG(glBindRenderbuffer(GL_RENDERBUFFER, m_renderBuffer.ID()));
    GL_DEBUG(glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, windowBounds.Width, windowBounds.Height));
    GL_DEBUG(glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_RENDERBUFFER, m_renderBuffer.ID()));
}
...
gol::GraphicsHandler::GridLineInfo gol::GraphicsHandler::CalculateGridLineInfo(Vec2, const GraphicsHandlerArgs& args) const
{
    const auto cameraCorner = Vec2F
    {
        Camera.Center.x - args.ViewportBounds.Width / 2.f / Camera.Zoom,
    ...
    const auto upperLeft = [cameraCorner, bounded, args] ()
    {
        const auto unboundedReturn = Vec2F
        {
            ...
            std::floor(cameraCorner.Y / args.CellSize.Height) * args.CellSize.Height
        };
    ...
    const auto lowerRight = [cameraCorner, bounded, args, this] ()
    {
        const auto unboundedReturn = Vec2F
        {
            cameraCorner.X + args.ViewportBounds.Width / Camera.Zoom,
        ...
        return Vec2F
        {
            ...
            std::min(unboundedReturn.Y, args.GridSize.Height * args.CellSize.Height)
        };
    }();
    ...
    const auto gridSize = [cameraCorner, upperLeft, bounded, args, this] ()
    {
        ...
        {
            static_cast<int32_t>(std::ceil(args.ViewportBounds.Width / Camera.Zoom / args.CellSize.Width)),
            static_cast<int32_t>(std::ceil(args.ViewportBounds.Height / Camera.Zoom / args.CellSize.Height))
        };

        if (!bounded)
            return unboundedReturn;
        
        const auto gridDimensions = Size2
        {
            args.GridSize.Width - static_cast<int32_t>(upperLeft.X / args.CellSize.Width),
            args.GridSize.Height - static_cast<int32_t>(upperLeft.Y / args.CellSize.Height)
		};

        return Size2
        {
            std::min(gridDimensions.Width, unboundedReturn.Width),
...
void gol::GraphicsHandler::DrawGridLines(Vec2 offset, const GraphicsHandlerArgs& args)
{
...
void gol::GraphicsHandler::CenterCamera(const GraphicsHandlerArgs& args)
{
    Camera.Center =
    {
        args.GridSize.Width / 2.f * args.CellSize.Width,
